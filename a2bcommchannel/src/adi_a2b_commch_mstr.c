...
static A2B_COMMCH_RET a2b_CommChEngMboxWriteCbk(void* commChHnd, uint8 nMboxNo, uint16 nNoOfBytes, uint8 *pWriteBuf, int8 nNodeAddr);
/*============= C O D E =============*/
/*****************************************************************************/
/*!
...
a2b_Handle adi_a2b_CommChMstrCreate(a2b_CommChMstrConfig *pCommChMstrConfig)
{
	...
	if(pCommChMstrConfig != A2B_NULL)
	{
		...
		pCommChMstrInfo->oCommChEngInfo.eFraming				= pCommChMstrConfig->eFraming;
	}
...
*/
/*****************************************************************************/
A2B_COMMCH_RET adi_a2b_CommChMstrTxMsg(a2b_Handle hCommChMstr, a2b_CommChMsg *pMsg , int8 nNodeAddr)
{
	...
	if (pCommChMstrInfo->eCommChMsgTxState == A2B_COMMCH_TX_IDLE)
	{
		...
		eRet = adi_a2b_CommChEngTxAction(&pCommChMstrInfo->oCommChEngInfo);
...
A2B_COMMCH_RET 	   adi_a2b_CommChMstrTick(a2b_Handle *hCommChMstr)
{
	a2b_CommChMstrInfo	*pCommChMstrInfo = (a2b_CommChMstrInfo*)hCommChMstr;
	a2b_CommChMsg		*pCommChTxMsg = &pCommChMstrInfo->oCommChEngInfo.oA2bTxMsg;
	a2b_CommChMsg		oTempCommChTxMsg, *pTempCommChTxMsg = &oTempCommChTxMsg;
	A2B_COMMCH_RET		 eRet = A2B_COMMCH_SUCCESS;
	...
	if(pCommChMstrInfo->eCommChMsgTxState == A2B_COMMCH_TX_BUSY)
	{
		...
		else if(pCommChMstrInfo->bTimeout == A2B_TRUE)
		{
			...
			/* Temporary copy the elements of a Communication Channel message before clearing the entire TxInfo
			 * This is required to let know the higher layers about which message got timed-out
			 */
			memcpy(pTempCommChTxMsg, pCommChTxMsg, sizeof(pCommChMstrInfo->oCommChEngInfo.oA2bTxMsg));
			...
			pCommChMstrInfo->oCommChEngInfo.pfStatCb(pCommChMstrInfo->oCommChEngInfo.pCbParam, pTempCommChTxMsg, A2B_COMMCH_EVENT_TX_TIMEOUT, pCommChMstrInfo->oCommChEngInfo.nTxNodeAddr);
		}
		else
		{
...
*/
/*****************************************************************************/
A2B_COMMCH_RET adi_a2b_CommChMstrSetFraming(a2b_Handle hCommChMstr, A2B_COMMCH_FRAMING eFraming)
{
	A2B_COMMCH_RET		 eRet = A2B_COMMCH_SUCCESS;
	a2b_CommChMstrInfo	 *pCommChMstrInfo;

	if(hCommChMstr != A2B_NULL)
	{
		pCommChMstrInfo = (a2b_CommChMstrInfo*)hCommChMstr;
		pCommChMstrInfo->oCommChEngInfo.eFraming = eFraming;
	}
	else
	{
		eRet = A2B_COMMCH_FAILED;
...
*/
/*****************************************************************************/
A2B_COMMCH_RET adi_a2b_CommChMstrGetFraming(a2b_Handle hCommChMstr, A2B_COMMCH_FRAMING *peFraming)
{
	A2B_COMMCH_RET		 eRet = A2B_COMMCH_SUCCESS;
	a2b_CommChMstrInfo	 *pCommChMstrInfo;

	if((hCommChMstr != A2B_NULL) && (peFraming != A2B_NULL))
	{
		pCommChMstrInfo = (a2b_CommChMstrInfo*)hCommChMstr;
		*peFraming = pCommChMstrInfo->oCommChEngInfo.eFraming;
	}
	else
	{
		eRet = A2B_COMMCH_FAILED;
	...
	if (msg)
	{
		...
		if (pMboxEventInfo)
		{
			...
			case A2B_MBOX_RX_DATA:
				...
				pCommChMstrInfo->oCommChEngInfo.nRxNodeAddr = (int8) pMboxEventInfo->nNodeId;
				adi_a2b_CommChEngRxAction(&pCommChMstrInfo->oCommChEngInfo, pMboxEventInfo->prBuf);
				break;
...
static A2B_COMMCH_RET a2b_CommChEngMboxWriteCbk(void* commChHnd, uint8 nMboxNo, uint16 nNoOfBytes, uint8 *pWriteBuf, int8 nNodeAddr)
{
	...
	pMailboxTxInfo->req.nDataSz 		= (uint8)nNoOfBytes;
