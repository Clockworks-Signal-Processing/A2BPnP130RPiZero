...
/** CommCh set framing request to a particular slave node */
#define A2B_MSGREQ_COMMCH_SET_FRAMING		(13u)

/** CommCh set framing request to a particular slave node */
#define A2B_MSGREQ_COMMCH_GET_FRAMING		(14u)

/** Start A2B slave node discovery */
#define A2B_MSGREQ_NODE_DISCOVERY           (15u)

/** Request directed to a slave plugin to complete
 * any necessary initialization of SPI peripherals
 * attached to the slave node.
 */
#define A2B_MSGREQ_PLUGIN_SPI_PERIPH_INIT   (16u)

 /** Partial discovery of dropped nodes */
#define A2B_MSGREQ_NET_POST_DISCOVERY       (17u)

#ifndef A2B_ENABLE_AD244xx_SUPPORT
#define A2B_MSGREQ_MAX                      (18u)
#else
#define A2B_MSGREQ_NET_CP_APIS              (18u)
#define A2B_MSGREQ_MAX                      (19u)
#endif
...
/*Boot message for the Remote tuner module*/
#define A2B_MSGREQ_RTM_BOOT					(102u)

/*Tune the remote tuner module*/
#define A2B_MSGREQ_RTM_TUNE                 (103u)

/*Mute the audio from the remote tuner module*/
#define A2B_MSGREQ_RTM_MUTE                 (104u)

/*Optimal response cycle calculation in progress */
#define A2B_MSGREQ_BLINDDISC_INPROG         (105u)

/*Optimal response cycle calculation completed */
#define A2B_MSGREQ_OPTIMAL_DISCCOMP         (106u)
...
/** Max message notify command (for NODE Discovery) */
#define A2B_MSGNOTIFY_NODE_DISCOVERY		(7u)

/** Notification that is emitted at the end of SPI to SPI peripheral configuration is finished for a slave node.
 * The notification is received for every node which has SPI to SPI peripheral configuration is required.
 *
 * For ex, 	A slave node with 3 SPI peripherals will get A2B_MSGNOTIFY_SPITOSPI_PERICONFIG_DONE only once for this node
 * 			when all the three SPI to SPI peripherals are configured.
 */
#define A2B_MSGNOTIFY_SPITOSPI_PERICONFIG_DONE	(8u)

 /** Peripheral I2C Error Reporting from Slave plugin */
#define A2B_MSGREQ_PERIPH_I2C_ERROR             (9u)

 /** Peripheral I2C Error Notification to application */
#define A2B_MSGREQ_PERIPH_I2C_ERROR_APP         (10u)

#ifdef A2B_ENABLE_AD244xx_SUPPORT
/*CP Interrupt*/
#define A2B_MSGNOTIFY_CPINTERRUPT            	(11u)

/** Max message notify command (for range checking) */
#define A2B_MSGNOTIFY_MAX                  		(12u)

#else	/* A2B_ENABLE_AD244xx_SUPPORT */

/** Max message notify command (for range checking) */
#define A2B_MSGNOTIFY_MAX                  		(11u)
...
typedef struct a2b_NetDiscovery
{
    ...
    struct {
        ...
        /** Pointer to the CP Reg Config Structure */
        a2b_Byte       *pCPRegConfig;
...
/** Payload data for A2B_MSGREQ_NET_POST_DISCOVERY within an a2bMsg request */
typedef struct a2b_NetPostDiscovery
{
    /** Input (request) parameters */
    struct {
        /** Enable partial discovery of bus */
        a2b_Bool      bIsPartialDiscEnabled;

        /** Dropped node in pevious discovery attempt or line fault detection */
        a2b_Int8      nBusDropNodeId;

        /* Confirmed post discovery line fault */
        a2b_Bool      bBusDropDetected;

#ifdef A2B_FEATURE_PNP
        /* PnP Node Discovery state */
        a2b_UInt8     PnPNodeDiscState;
#endif
    } req;
...
/** Payload data for an A2B_MSGREQ_COMMCH_FRAMING message within a
 * a2bMsg message.
 */
typedef struct a2b_CommChFramingInfo
{
	/** Input (request) parameters */
	    struct {
...
/*Payload data for RTM within a2bMsg request*/
typedef struct a2b_rtmTune
{
	struct
	{
		/*FM tune frequency for the radio module*/
		a2b_UInt32 nTuneFreq;

		/*Mute or unmute command for the RTM*/
		a2b_Bool bMute;
	}tune;
}a2b_rtmTune;
#endif
...
#ifdef A2B_ENABLE_AD244xx_SUPPORT
/** Payload data for A2B_MSGREQ_NET_CP_APIS within an a2bMsg request */
typedef struct a2b_Net_CP_APIs
{
    /** Input (request) parameters */
    struct {
        /** enumeration for the CP API
         */
    	ADI_A2B_CP_CMD 	eCPApiEnum;

        /** NodeAddress the API is meant for */
        a2b_Int32      nNodeAddr;
...
/**
 * This is the notification payload for both A2B_MSGNOTIFY_NODE_DISCOVERY notification.
 */
typedef struct a2b_Nodedscvry
{
    /** Pointer to BDD network that was used for discovery.
     */
	const void		*bddNetObj;

    /**
     * The A2B node address of the node that was just discovered.
     */
    a2b_Int16       nodeAddr;

    /**
     * Discovery success/failure indication
     */
    a2b_UInt32      discoveryCompleteCode;

    /**
     * This shuld be populated by application to continue/end discovery process
     * A2B_TRUE  : Continue discovery
     * A2B_FALSE : End discovery
     */
 ...
 * A2B_MSGNOTIFY_INTERRUPT notifications.
 */
typedef struct a2b_CPInterrupt
{
    /** The reported interrupt type (per A2B_ENUM_INTTYPE_PWRERR_XXX
     * enumeration defined in a2b/regdefs.h).
     */
    ...
    /** The reported interrupt type (per A2B_ENUM_INTTYPE_PWRERR_XXX
      * enumeration defined in a2b/regdefs.h).
      */
    a2b_UInt8       aCPPortintrType[A2B_MAX_CP_PORTS];
...
typedef struct a2b_CommChEventInfo{
	...
	/*! Framing enabled or disabled information to user */
	A2B_COMMCH_FRAMING	eFraming;
...
/**
 * This is the notification payload for A2B_MSGREQ_PERIPH_I2C_ERROR_APP notifications.
 */
typedef struct a2b_I2CError
{
    /** Node address */
    a2b_Int8        nodeAddr;

    /** Peripheral address where I2C Error occurred */
    a2b_UInt8       I2CAddr;
} a2b_I2CError;
