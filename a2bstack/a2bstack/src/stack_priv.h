...
typedef   enum {
    	/** SPI Access from Unknown source */
		A2B_SPI_ACCESS_UNKNOWN = (a2b_UInt32)0u,
		/** SPI Access from Master Plugin */
        A2B_SPI_ACCESS_MASTER,
		/** SPI Access from Slave Plugin */
        A2B_SPI_ACCESS_SLAVE,
		/** SPI Access from Remote peripheral */
        A2B_SPI_ACCESS_PERIPH
} ADI_A2B_SPI_ACCESS_MODE;
...
/**
 * Used to store that last SPI access of the stack
 */
typedef struct a2b_SpiLastMode
{
	ADI_A2B_SPI_ACCESS_MODE     access;
	a2b_Int16   nodeAddr;
    a2b_UInt16  chipAddr;
    a2b_Bool    broadcast;
} a2b_SpiLastMode;
...
typedef struct a2b_Stack
{
	...
	/** The handle to the open SPI device */
	a2b_Handle                  spiHnd;

	/** The handle for file operation */
	a2b_Handle                  fp;

    /** Used to remember that last I2C access mode for the stack */
    a2b_I2cLastMode             i2cMode;

    /** Used to remember that last SPI access mode for the stack */
    a2b_SpiLastMode             spiMode;

    /** The handle to the open audio device */
    a2b_Handle                  audioHnd;

	ADI_A2B_ACCESS_INTERFACE    accessInterface;
    ...
    /** Initialization mask of sub-systems */
    a2b_SpiInfo					oSpiInfo;

    a2b_UInt8					bSpiIntOnGpio;
} a2b_Stack;
...
A2B_EXPORT A2B_DSO_LOCAL void a2b_stackResetSpiLastMode(a2b_SpiLastMode* lastMode);
#ifdef A2B_BCF_FROM_FILE_IO
A2B_EXPORT A2B_DSO_LOCAL a2b_HResult a2b_stackFileRead(struct a2b_StackContext*    ctx,
	    a2b_UInt16                  nOffset,
	    a2b_UInt16                  nRead,
	    void*                       rBuf
	    );
