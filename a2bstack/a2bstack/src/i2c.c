...
*  \b   a2b_i2cSlaveRead
* 
*  Reads bytes from the slave node. Only the master plugin can
*  issue a read request. All other slave plugins and applications requesting
*  to read a slave node will fail. The read operation will continue at the
...
* 
*  Writes bytes to all the slave nodes. Only the master plugin can issue a
*  broadcast write request. All other slave plugins and applications requesting
*  to broadcast write to slave nodes will fail. The first byte of the buffer is
*  treated as the AD2410 register offset by the chip. For subsequently written
*  bytes the chip will auto-increment the register offset with each byte
*  written. This is an synchronous call and will block until the operation is
*  complete.
* 
*  \param   [in]        ctx         The stack context associated with the
*                                   write.
   ...
   )
{
    ...
    if ( (A2B_NULL != ctx) &&
        ((A2B_NULL != wBuf) || (nWrite == 0u)) )
    {
        /* Only the master plugin is allowed to write to slave nodes */
        if ( (ctx->domain != A2B_DOMAIN_PLUGIN) ||
            (ctx->ccb.plugin.nodeSig.nodeAddr != A2B_NODEADDR_MASTER) )
        {
            result = A2B_MAKE_HRESULT(A2B_SEV_FAILURE,
