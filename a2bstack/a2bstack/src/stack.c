 ...
 *  \b              a2b_stackFileRead
 *
 *  Reads binary file for IO operation
 *
 *
 *
 *  \param          [in]    ctx      Pointer to Stack context
 *  \param          [in]    nOffset   Number of bytes to skip
 *  \param          [in]    nRead   Number of bytes to read
 *  \param          [in]    rBuf   Read buffer
 *
 *  \pre            None
 *
 *  \post           None
...
{
    return(ctx->stk->pal.fileRead(ctx->stk->fp,
    								 nOffset,
                                     nRead, rBuf));
}
...
/*!****************************************************************************
 *
 *  \b              a2b_stackSetFileIO
 *
 *  Set FILE pointer for IO operation
 *
 *
 *
 *  \param          [in]    stk      Pointer to Stack context
...
/*!****************************************************************************
 *
 *  \b              a2b_stackResetSpiLastMode
 *
 *  Resets the SPI last access "mode" to default (uninitialized) values. These
 *  values will cause the SPI read/write routines to re-initialize the A2B
 *  SPI access registers and skip their (assumed) cached values.
 *
 *  \param          [in]    lastMode     The last access mode parameters to
 *                                       be reset.
 *
 ...
 ******************************************************************************/
A2B_DSO_LOCAL void
a2b_stackResetSpiLastMode
(
		a2b_SpiLastMode*    lastMode
)
{
	if ( A2B_NULL != lastMode )
	{
		lastMode->access 	= A2B_SPI_ACCESS_UNKNOWN;
		lastMode->nodeAddr 	= A2B_NODEADDR_NOTUSED;
		lastMode->chipAddr 	= A2B_INVALID_7BIT_I2C_ADDRESS;
		lastMode->broadcast = A2B_FALSE;
	}
...
)
{
	...
	if ( A2B_NULL != stk )
	{
		...
		if ( stk->initMask & A2B_INITIALIZED_I2C )
		{
			...
			if ( A2B_NULL != stk->spiHnd )
			{
				if ( A2B_NULL != stk->pal.spiClose )
				{
					stk->pal.spiClose(stk->spiHnd);
				}
			}

			if ( A2B_NULL != stk->pal.spiShutdown )
			{
				stk->pal.spiShutdown(stk->ecb);
			}
...
)
{
	...
	if ( (A2B_NULL != pal) && (A2B_NULL != ecb) )
	{
			...
			if ( A2B_NULL != stk->pal.i2cInit )
			{
				...
				if ( A2B_SUCCEEDED(status) )
				{
					...
					else
					{
						...
						stk->accessInterface = A2B_ACCESS_I2C;
					}
				}
			}
		}
		...
		if (A2B_SUCCEEDED(status))
		{
			if (A2B_NULL != stk->pal.spiInit)
			{
				status = stk->pal.spiInit(stk->ecb);
				if (A2B_SUCCEEDED(status))
				{
					stk->initMask |= A2B_INITIALIZED_I2C;
					stk->spiHnd = stk->pal.spiOpen(stk->ecb);
					if (A2B_NULL == stk->spiHnd)
					{
						status = A2B_MAKE_HRESULT(A2B_SEV_FAILURE,
								A2B_FAC_SPI,
					...
					{
						/* If both are present make choice  */
						stk->accessInterface = A2B_ACCESS_SPI;
					}
				}
			}
		}
...
/*!****************************************************************************
 *
 *  \b              a2b_stackSetAccessInterface
 *
 *  Set access interface, either SPI or I2C
 *
 *
 *
 *  \param          [in]    stk      Pointer to Stack context
 *  \param          [in]    access   Access interface (0- I2C, 1 -PI)
 *
 *  \pre            None
 *
 *  \post           None
...
)
{
	if (A2B_NULL != ctx)
	{
		ctx->stk->accessInterface = access;
	}
} /* a2b_setSpiIntOnGpioStat */

/*!****************************************************************************
 *
 *  \b              a2b_setSpiIntOnGpioStat
 *
 *  Sets the flag when SPI interrupts are routed on GPIO line
 *
 *
 *
 *  \param          [in]    stk      Pointer to Stack context
 *  \param          [in]    nRegVal  Value of SPIPINCFG register
 *
 *  \pre            None
 *
 *  \post           None
...
)
{
	if (A2B_NULL != ctx)
	{
		ctx->stk->bSpiIntOnGpio = nFlagVal;
	}
} /* a2b_setSpiIntOnGpioStat */
...
/*!****************************************************************************
 *
 *  \b              a2b_stackGetAccessInterface
 *
 *  Get access interface, either SPI or I2C
 *
 *
 *
 *  \param          [in]    stk      Pointer to Stack context
...
{
	ADI_A2B_ACCESS_INTERFACE  access = A2B_ACCESS_I2C;
	if (A2B_NULL != ctx)
	{
		access = ctx->stk->accessInterface;
	}
	return(access);
} /* a2b_stackSetAccessInterface */
...
/*!****************************************************************************
 *
 *  \b              a2b_stackPalGetPGSignal
 *
 *  Returns the Power Good signal reqired for Two Step Discovery.
 *
 *  \param       [in]       ctx          The A2B stack instance.
 *
 *  \param       [in,out]   IsPresent    Return value
 *
 *  \pre         None.
 *
 *  \post        None
...
)
{
	if (A2B_NULL != ctx)
	{
		ctx->stk->pal.getPGSignal(ctx->stk->i2cHnd, ctx->stk->ecb->baseEcb.i2cMasterAddr, IsPresent);
	}
} /* a2b_stackPalGetPGSignal */
#endif
...
/*!****************************************************************************
 *
 *  \b              a2b_stackResetSpiToSpiPeriReqRespFlags
 *
 *  This function should be called to reset the SPI to SPI peripheral configuration
 *  request and response counters and flags.
 *
 *  \param          [in]    ctx      The A2B stack instance.
 *
 *  \pre            None
...
A2B_DSO_PUBLIC void a2b_stackResetSpiToSpiPeriReqRespFlags(struct a2b_StackContext* ctx)
{
	...
	if (A2B_NULL != ctx)
	{
		...
		{
			/* Reset the SPI to SPI peripheral configuration request & response flags for slave nodes */
			ctx->stk->oSpiInfo.nSpiToSpiPeriConfigReq 			 = 0U;
			ctx->stk->oSpiInfo.fSpiToSpiPeriConfigReq[nodeAddr]  = A2B_FALSE;

			ctx->stk->oSpiInfo.nSpiToSpiPeriConfigResp 			 = 0U;
			ctx->stk->oSpiInfo.fSpiToSpiPeriConfigResp[nodeAddr] = A2B_FALSE;
		}
	}
}
...
/*!****************************************************************************
 *
 *  \b              a2b_stackIncrSpiToSpiPeriConfigReqFlags
 *
 *  This function should be called to increment the SPI to SPI peripheral configuration
 *  request counters and flags.
 *
 *  \param          [in]    ctx      The A2B stack instance.
 *
 *  \param          [in]    nodeAddr	Slave Node address.
 *
 *  \pre            None
 *
 *  \post           None
...
A2B_DSO_PUBLIC void a2b_stackIncrSpiToSpiPeriConfigReqFlags(struct a2b_StackContext* ctx, a2b_Int16 nodeAddr)
{
	if (A2B_NULL != ctx)
	{
		...
		ctx->stk->oSpiInfo.nSpiToSpiPeriConfigReq++;
		ctx->stk->oSpiInfo.fSpiToSpiPeriConfigReq[nodeAddr]  = A2B_TRUE;
	}
}

/*!****************************************************************************
 *
 *  \b              a2b_stackIncrSpiToSpiPeriConfigRespFlags
 *
 *  This function should be called to increment the SPI to SPI peripheral configuration
...
A2B_DSO_PUBLIC void a2b_stackIncrSpiToSpiPeriConfigRespFlags(struct a2b_StackContext* ctx, a2b_Int16 nodeAddr)
{
	if (A2B_NULL != ctx)
	{
		...
		ctx->stk->oSpiInfo.nSpiToSpiPeriConfigResp++;
		ctx->stk->oSpiInfo.fSpiToSpiPeriConfigResp[nodeAddr] = A2B_TRUE;
	}
}

/*!****************************************************************************
 *
 *  \b              a2b_stackChkSpiToSpiPeriConfigReqRespCnt
 *
 *  This function should be called to verify if the SPI to SPI peripheral configuration
 *  request and response counters are equal.
 *
 *  \param          [in]    ctx      The A2B stack instance.
 *
 *  \pre            None
 ...
 *  \post           None
 *
 *  \return         A2B_TRUE: If request and response counters are equal.
 *  				   A2B_FALSE: If request and response counters are not equal.
 *
 ******************************************************************************/
A2B_DSO_PUBLIC a2b_Bool a2b_stackChkSpiToSpiPeriConfigReqRespCnt(struct a2b_StackContext* ctx)
{
	a2b_Bool SpiToSpiPeriConfigReqRespCntEqual = A2B_FALSE;

	if (A2B_NULL != ctx)
	{
		if(ctx->stk->oSpiInfo.nSpiToSpiPeriConfigReq == ctx->stk->oSpiInfo.nSpiToSpiPeriConfigResp)
		{
			SpiToSpiPeriConfigReqRespCntEqual = A2B_TRUE;
		}
	}
	return (SpiToSpiPeriConfigReqRespCntEqual);
}

/*!****************************************************************************
 *
 *  \b              a2b_stackChkSpiToSpiPeriConfigReqRespFlags
 *
 *  This function should be called to verify if the SPI to SPI peripheral configuration
 *  request and response flags are equal.
 *
 *  \param          [in]    ctx      The A2B stack instance.
 *
 *  \param          [in]    nodeAddr	Slave Node address.
 ...
 *  \post           None
 *
 *  \return         A2B_TRUE: If request and response flags are equal.
 *  				   A2B_FALSE: If request and response flags are not equal.
 *
 ******************************************************************************/
A2B_DSO_PUBLIC a2b_Bool a2b_stackChkSpiToSpiPeriConfigReqRespFlags(struct a2b_StackContext* ctx, a2b_Int16 nodeAddr)
{
	a2b_Bool SpiToSpiPeriConfigReqRespFlagEqual = A2B_FALSE;

	if (A2B_NULL != ctx)
	{
		if(ctx->stk->oSpiInfo.fSpiToSpiPeriConfigReq[nodeAddr] == ctx->stk->oSpiInfo.fSpiToSpiPeriConfigResp[nodeAddr])
		{
			SpiToSpiPeriConfigReqRespFlagEqual = A2B_TRUE;
		}
	}
	return (SpiToSpiPeriConfigReqRespFlagEqual);
}
