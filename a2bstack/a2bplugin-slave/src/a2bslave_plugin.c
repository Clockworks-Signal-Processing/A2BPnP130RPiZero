    ...
    )
{
    ...
    a2b_Plugin*                 masterPlugin;
    a2b_PluginInit*             initMsg;
    a2b_Int16                   nodeAddr= a2b_msgGetDestNodeAddr(msg);
    a2b_HResult 				nRes;
    struct a2b_Msg* 			msgSpiToSpiPeriConfig;
#ifdef ENABLE_PERI_CONFIG_BCF
     const ADI_A2B_NODE_PERICONFIG ( *pPeriConfig)[];  /* Pointer to an array of constant configuration */
#endif
    ...
    switch ( cmd )
    {
        case A2B_MSGREQ_PLUGIN_PERIPH_INIT:
            ...
            {
            	pPeriConfig = ( const ADI_A2B_NODE_PERICONFIG (*)[])initMsg->req.pNodePeriDeviceConfig;
            	plugin->pNodePeriDeviceConfig = &((*pPeriConfig)[((a2b_UInt32)nodeAddr + (a2b_UInt32)1)]);

            	nRes = adi_a2b_PeriheralConfig(plugin, plugin->pNodePeriDeviceConfig, A2B_FALSE);
            }
#endif
#else
			if( plugin->ctx->stk->ecb->palEcb.oAppEcbPal.periConfigDuringDisc != A2B_NULL)
			{
                plugin->ctx->stk->ecb->palEcb.oAppEcbPal.periConfigDuringDisc(nodeAddr);
			}
			if (plugin->ctx->stk->ecb->palEcb.oAppEcbPal.PostCustomNodeConfig != A2B_NULL)
			{
				plugin->ctx->stk->ecb->palEcb.oAppEcbPal.PostCustomNodeConfig(nodeAddr);
			}
#endif
        ...
        case A2B_MSGREQ_PLUGIN_SPI_PERIPH_INIT:
            initMsg = (a2b_PluginInit*)a2b_msgGetPayload( msg );
            initMsg->resp.status = A2B_RESULT_SUCCESS;
#ifndef A2B_SS_STACK
#ifdef ENABLE_PERI_CONFIG_BCF
            if(initMsg->req.pNodePeriDeviceConfig != A2B_NULL)
            {
            	...
            	nRes = adi_a2b_PeriheralConfig(plugin, plugin->pNodePeriDeviceConfig, A2B_TRUE);
            }
#endif
#else

			if (plugin->ctx->stk->ecb->palEcb.oAppEcbPal.periConfigPostDisc != A2B_NULL)
			{
				plugin->ctx->stk->ecb->palEcb.oAppEcbPal.periConfigPostDisc(nodeAddr);
			}
#endif
			if ( initMsg->req.pNodePeriDeviceConfig )
			{
				A2B_TRACE2((ctx, (A2B_TRC_DOM_PLUGIN | A2B_TRC_LVL_INFO),
						"Slave%hd Plugin: "
						"A2B_MSGREQ_PLUGIN_SPI_PERIPH_INIT processed (pNodePeriDeviceConfig: %hu)",
						&nodeAddr,
						&plugin->pNodePeriDeviceConfig));
				A2B_SEQ_RAW2(ctx,  A2B_SEQ_CHART_LEVEL_DISCOVERY,
								"== Slave%hd Plugin: "
								"A2B_MSGREQ_PLUGIN_SPI_PERIPH_INIT "
								"Processed (pNodePeriDeviceConfig: %hu) ==",
								&nodeAddr,
								&plugin->pNodePeriDeviceConfig);
			}


			/* Getting master plugin handle */
			masterPlugin = (a2b_Plugin*)a2b_msgGetUserData(msg);

			/* Notify listeners that SPI to SPI peripheral configuration is done */
			msgSpiToSpiPeriConfig = a2b_msgAlloc( plugin->ctx, A2B_MSG_NOTIFY, A2B_MSGNOTIFY_SPITOSPI_PERICONFIG_DONE );
			if ( A2B_NULL == msgSpiToSpiPeriConfig )
			{
				A2B_TRACE1( (plugin->ctx, (A2B_TRC_DOM_PLUGIN | A2B_TRC_LVL_ERROR),
							"%s a2b_dscvryEnd(): Failed to allocate SPITOSPI_PERICONFIG_DONE "
							"notification msg", A2B_SLAVE_PLUGIN_NAME));
			}
			else
			{
				A2B_TRACE1( (plugin->ctx, (A2B_TRC_DOM_PLUGIN | A2B_TRC_LVL_ERROR),
											"%s a2b_dscvryEnd(): Success to allocate SPITOSPI_PERICONFIG_DONE "
											"notification msg", A2B_SLAVE_PLUGIN_NAME));

				/* Setting master plugin handle as user data in notify message */
				a2b_msgSetUserData(msgSpiToSpiPeriConfig, masterPlugin, A2B_NULL);

				/* Sending the current slave node address */
				a2b_msgSetTid(msgSpiToSpiPeriConfig, nodeAddr);

				/* Make best effort delivery of notification */
				nRes = a2b_msgRtrNotify(msgSpiToSpiPeriConfig);
				if ( A2B_FAILED(nRes) )
				{
				    A2B_TRACE1((plugin->ctx, (A2B_TRC_DOM_PLUGIN | A2B_TRC_LVL_ERROR),
										"Exit: %s:  SPI To SPI Peripheral Configuration Done Notification failed",
										A2B_SLAVE_PLUGIN_NAME));
				}
				else
				{
					A2B_TRACE1((plugin->ctx, (A2B_TRC_DOM_PLUGIN | A2B_TRC_LVL_ERROR),
															"Exit: %s:  SPI To SPI Peripheral Configuration Done Notification success",
															A2B_SLAVE_PLUGIN_NAME));
				}

				(void)a2b_msgUnref(msgSpiToSpiPeriConfig);
			}
