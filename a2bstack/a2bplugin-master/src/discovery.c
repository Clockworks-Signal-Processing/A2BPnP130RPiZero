...
#if defined(A2B_FEATURE_PNP) && defined(A2B_FEATURE_OPTIMAL_RESPCYCS)
typedef enum
{
...
static void a2b_UpdateSlotConfiguration(a2b_Plugin* plugin);
/*!****************************************************************************
*
    ...
    )
{
...
#if defined(A2B_FEATURE_PNP) && defined(A2B_FEATURE_OPTIMAL_RESPCYCS)
    if(plugin->discovery.PnPNodeDiscState == A2B_PNP_DISCSTATE_OPT)
    {
    	/* Looks like discovery failed while attempting a discovery with optimal response cycle for unknown cable length
    	 * Reset the state */
    	plugin->discovery.PnPNodeDiscState = A2B_PNP_DISCSTATE_BD;
    }
#endif
	...
	if ((isAd243x) && (bddNodeObj->nodeSetting.eHighPwrSwitchCfg == bdd_highPwrSwitchCfg_HPSW_CFG_4) && (A2B_SUCCEEDED(ret)) && (plugin->discovery.bAd243xCfg4OpenDetect == false))
	{
		...
		a2b_Int16   nodeAddr)
{
    ...
    if ((A2B_NODEADDR_MASTER == nodeAddr) && (plugin != A2B_NULL))
    {
		...
		/*  Request for new slot configuration from Plug and Play Library
			Callback to Plug and Play library */
		if(plugin->bdd->nodes_count > 2)
		{
            a2b_UpdateSlotConfiguration(plugin);
		}
#endif
...
#ifndef A2B_FEATURE_EEPROM_OR_FILE_PROCESSING
#ifndef A2B_FEATURE_PNP /* TODO: SPI peripherals are not supported yet in PnP feature */
    ...
    )
{
...

   {
	...
	else
	{
		...
		if( (plugin->discovery.PnPNodeDiscState != A2B_PNP_DISCSTATE_BD) && (plugin->discovery.nNodeOptDiscIdx != dscNodeBddIdx))
		{
			plugin->discovery.PnPNodeDiscState = A2B_PNP_DISCSTATE_BD;
		}

		if(plugin->discovery.PnPNodeDiscState == A2B_PNP_DISCSTATE_BD)
		{
			/* Blind Discovery: Subtract the standard 4 response cycles from the previous node and try discovery */
			if(!plugin->discovery.bBDSecondPass)
			{
				plugin->discovery.nRespCycleCounter = (a2b_UInt8)plugin->bdd->nodes[dscNodeBddIdx].ctrlRegs.respcycs - A2B_PNP_RESP_CYCLES_MIN_DIFF;
				plugin->discovery.bBDSecondPass = 1;
			}
			else
			{
				/* Tried beyond the allowed difference between the response cycles of two consecutive nodes and still seeing the failure,
				 * Mostly it is unsupported cable length or no new node is connected. Switch back to default response cycle and try again */
				if((plugin->bdd->nodes[dscNodeBddIdx].ctrlRegs.respcycs - plugin->discovery.nRespCycleCounter) >= A2B_PNP_RESP_CYCLES_MAX_DIFF)
				{
					/* Either a long cable or open */
					plugin->discovery.nRespCycleCounter = (a2b_UInt8)plugin->bdd->nodes[dscNodeBddIdx].ctrlRegs.respcycs - A2B_PNP_RESP_CYCLES_MIN_DIFF;
					plugin->discovery.bBDSecondPass = 0;
				}
				else
				{
					/* Keep trying for successful discovery by decrementing response cycles */
					plugin->discovery.nRespCycleCounter--;
				}
			}
			wBuf[1] = (a2b_UInt8)plugin->discovery.nRespCycleCounter;
		}
		else if(plugin->discovery.PnPNodeDiscState == A2B_PNP_DISCSTATE_OPT)
		{
			/* Re-discovery with optimal Response cycle */
			plugin->discovery.nRespCycleCounter+=A2B_PNP_RESP_CYCLES_INC_FOR_OPTIMAL;
			plugin->bdd->nodes[dscNodeBddIdx + 1].ctrlRegs.respcycs = plugin->discovery.nRespCycleCounter;
			wBuf[1] = plugin->discovery.nRespCycleCounter;
		}
#else
    ...
    )
{
    ...
    a2b_UInt8 	rBuf[8u];
#if defined(A2B_FEATURE_PNP) && defined(A2B_FEATURE_OPTIMAL_RESPCYCS)
	a2b_UInt8 	CurRespCycle;
    a2b_UInt8 	OriRespCycle;
#endif
    a2b_Int16	 idx;
    a2b_HResult status = A2B_RESULT_SUCCESS;
    ...
    /* Only applicable from second sub node */
	if(dscNodeAddr >= 1)
	{
		/* Initiate the calculation of optimal response cycle for unknown cable length */
		if( (plugin->discovery.PnPNodeDiscState == A2B_PNP_DISCSTATE_BD))
		{
			/* Response cycle with which last discovery of new node was initiated */
			CurRespCycle = plugin->discovery.nRespCycleCounter;
			/* Response cycle of the upstream node */
			OriRespCycle = plugin->bdd->nodes[dscNodeAddr].ctrlRegs.respcycs;

			/* Try to get the failure within defined range */
			while( (OriRespCycle-CurRespCycle) <= A2B_PNP_RESP_CYCLES_MAX_DIFF)
			{
				/* Keep Decrementing the response cycle for new node until failure is found */
				plugin->discovery.nRespCycleCounter--;
				CurRespCycle = plugin->discovery.nRespCycleCounter;
				/* Write new response cycle */
				wBuf[0] = A2B_REG_RESPCYCS;
				wBuf[1] = CurRespCycle;
				status = a2b_regWrite(ctx, dscNodeAddr, 2, wBuf);

				/* Write NEW STRUCT bit to apply the response cycle */
				wBuf[0] = A2B_REG_CONTROL;
				status  = a2b_regWriteRead( ctx, -1, 1u, wBuf, 1u, rBuf );
				wBuf[1] = rBuf[0] | A2B_ENUM_CONTROL_START_NS;
				status = a2b_regWrite(ctx, -1, 2, wBuf);

				/* Read the response cycle back to make sure that new node is not dropped */
				wBuf[0] = A2B_REG_RESPCYCS;
				status  = a2b_regWriteRead( ctx, dscNodeAddr, 1u, wBuf, 1u, rBuf );

				if ( A2B_SUCCEEDED(status) )
				{
					/* Continue until failure is seen or response cycle is gone out of range */
					continue;
				}
				else
				{
					/* New node dropped: Found response cycle where access to new node fails */
					break;
				}

			}

			/* Set the state that re-discovery is initiated with optimal response cycles */
			plugin->discovery.PnPNodeDiscState = A2B_PNP_DISCSTATE_OPT;

			plugin->discovery.bBDSecondPass = 0;
			plugin->discovery.nNodeOptDiscIdx = dscNodeAddr;
	...
	/* Reset the Optimal response cycle state for node with unknown cable length */
	plugin->discovery.PnPNodeDiscState = A2B_PNP_DISCSTATE_BD;
#endif
...
static a2b_Bool a2b_dscvryCustomAuthFrmMem(a2b_Plugin* plugin, a2b_NodeSignature nodeSig)
{
	...
	a2b_UInt8 	nIdx, nIdxfail, rBufCustomNodeId[50u];
    a2b_Int16 	dscNodeAddr = (a2b_Int16)plugin->discovery.dscNumNodes;
    a2b_Int16 	dscNodeIdx = dscNodeAddr+1;
	a2b_Bool 	bContinueDisc;
	const bdd_Node					*bddNodeObj;
...
static void a2b_UpdateSlotConfiguration(a2b_Plugin* plugin)
{
    ...
    a2b_UInt32 				i2cCount = 0u;
    a2b_UInt8 				wBuf[4u];
	a2b_Int16 nodeIdx = 0;   /* TODO Master not Included */
	a2b_Int16 nodeAddr = 0;  /* TODO Master not Included */

	nodeAddr = plugin->bdd->nodes_count-2;

	for(nodeIdx=plugin->bdd->nodes_count-1; nodeIdx>0; nodeIdx--)
	{
