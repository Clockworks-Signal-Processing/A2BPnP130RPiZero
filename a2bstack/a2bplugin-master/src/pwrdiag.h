...
typedef enum {
	...
	/** The power diagnosis is in the process of executing the detection
	* of wrongly reported faults when a AD243x node is present in network.
	*/
	A2B_PWR_DIAG_STATE_IN_PROGRESS_AD243x,
} a2b_PwrDiagState;
...
typedef struct a2b_PwrDiagResults
{
	...
	/** Information which will be used to perform partial bus operation when a non-critical fault is found 
		Non-Critical faults: Open, Wrong Port, Reverse wires, Defective node, Short of Wires
	*/
	a2b_Bool                    isNonCriticalFault;
...
typedef struct a2b_PwrDiagCtx
{
	...
	/* For given node, upstream node with self power. Useful while shutting off the bus during critical fault */
	a2b_Int16					upstrSelfPwrNode;
...
A2B_EXPORT a2b_Bool a2b_pwrDiagIsActiveAD243x(struct a2b_Plugin* plugin);
A2B_END_DECLS
