...
typedef enum
{
    ...
    CFGBLK_TYPE_B,
    CFGBLK_TYPE_C,
	CFGBLK_TYPE_G = 7u /* CFG type used for Peripheral Read through DAT File/EEPROM */
} a2b_cfgBlockType;
...
static void a2b_periMsgDestroy(a2b_Plugin* plugin, a2b_Int16 nodeAddr);
/*!****************************************************************************
*
    ...
    )
{
    ...
    if ( bdd_CONFIG_METHOD_HYBRID == a2b_ovrGetDiscCfgMethod(plugin) )
    {
        ...
        status = a2b_periphWriteRead( plugin->ctx,
        								 A2B_NODEADDR_MASTER,
                                         A2B_I2C_EEPROM_ADDR,
                                         nWrite, wBuf,
                                         nRead, rBuf );
#elif defined A2B_BCF_FROM_FILE_IO
        A2B_GET_UINT16_BE(offset, (a2b_UInt8*)wBuf, 0u);
        status = a2b_stackFileRead(plugin->ctx,
        								 (a2b_UInt16)offset,
                                         nRead, rBuf);
#else
    ...
    )
{
    ...
    a2b_PeripheralNode* periphNode;
    a2b_UInt8 regAddr;
    a2b_UInt8 spiHeaderPresent=false;
    a2b_UInt8 spiUnitCount;
    a2b_UInt8 datExportVer;
    a2b_UInt8 nDataWidth = 1u;
    a2b_UInt8 nAddressWidth;
    a2b_UInt8 nRead;
    ...
    /* Read DAT export version - this shall be made part of BDD graph instead of accessing here */
    wBuf[0] = 0x00;
    wBuf[1] = 0x07;
#ifdef A2B_BCF_FROM_SOC_EEPROM
    /* Reading from EEPROM connected to SOC */
         ...
         periphNode->cfgIdx++)
    {
        ...
        /* Check if this is SPI header
         * bIsPreSpiWr is set or reset based on if this is SPI or I2C transaction and is used in next iteration - bIsPreSpiWr takes care
         * of delay commands of SPI peripherals. Delay header does not have information if it is for SPI or I2C peripheral, so last command is used as reference.
         * Assumption is that delay will never be the first command in any peripheral configuration	 */
        if( ((cfgType != CFGBLK_TYPE_C) && (plugin->periph.rBuf[2] & 0x80)) || ((cfgType == CFGBLK_TYPE_C) && plugin->periph.bIsPreSpiWr) )
        {
        	spiHeaderPresent = true;
        	spiUnitCount = plugin->periph.spiUnitCount[nodeAddr];

        	/* Check if allocated number of SPI peripheral configuration units is already crossed */
        	if(plugin->periph.spiUnitCount[nodeAddr] > A2B_CONF_MAX_NUM_SPI_UNITS_PER_NODE)
        	{
				A2B_DSCVRY_ERROR1( plugin->ctx, "periphCfgProcessing",
								   "Number of SPI Peripheral units crossed A2B_CONF_MAX_NUM_SPI_UNITS_PER_NODE",
								   &periphNode->cfgIdx );
				A2B_DSCVRY_SEQEND( plugin->ctx );
				A2B_DSCVRY_SEQEND( plugin->ctx );
				A2B_DSCVRY_SETERROR( plugin, A2B_EC_SPITOSPI_PERICONFIG_FAILED );
				return A2B_EXEC_COMPLETE;
        	}
        ...
        {
        	/* Reset the flag as this is I2c access */
        	plugin->periph.bIsPreSpiWr = 0u;
        }
        ...
        {
        	/* continue only if this is I2C write */
        	if(spiHeaderPresent == false)
        	{
        ...
        else
        {
        	...
        	if(spiHeaderPresent == false)
            {
	            ...
	            if ( !bA2bReg )
	            {
	            	if ( (a2b_UInt8)CFGBLK_TYPE_G == cfgType )
	            	{
	            		/* Need a master plugin context to do I2C/SPI calls */
	            		 	mCtx = (a2b_StackContext*)a2b_stackContextFind(plugin->ctx, A2B_NODEADDR_MASTER);
	            		 	if ( A2B_NULL == mCtx)
	            		 	{
	            		 	...
	            		 	else
	            		 	{
	            		 		...
	            		 		{
	            		 			nAddressWidth = plugin->periph.rBuf[A2B_OFFSET_DAT_ADDRWDTH];
									nRead = (payloadDataLen - A2B_OFFSET_DAT_ADDR - nAddressWidth) * nDataWidth;
									(void)a2b_memset(&aDataBuffer[0u], (a2b_Int32)0u, (a2b_Size)nRead);
									/* Buffer address is offset to the the actual Address */
									status = a2b_i2cPeriphWriteRead( mCtx, nodeAddr, (a2b_UInt16)regAddr, nAddressWidth, &plugin->periph.rBuf[A2B_OFFSET_DAT_ADDR], nRead, &aDataBuffer[0u]);
	            		 		}
	            		 		else
	            		 		{
	            		 			status = a2b_SpiCmdsAndPeriWriteRead(mCtx, nodeAddr, A2B_CMD_SPI_REMOTE_I2C_READ_REQUEST, (a2b_UInt16)regAddr, payloadDataLen, (void*)&plugin->periph.rBuf, 0u, A2B_NULL , A2B_MAX_LENINBYTES_REMOTESPITOI2C);
	            		 		}
	            	...
	            	{
	            		status = a2b_periphWrite(plugin->ctx,
	            			                                             nodeAddr,
	            			                                             (a2b_UInt16)regAddr,
	            														 datExportVer,
	            														 payloadDataLen,
	            ...
	            else
	            {
	                /* Write to the A2B Slave */
	              	status=a2b_regWrite(plugin->ctx,
											nodeAddr,
											payloadDataLen,
											plugin->periph.rBuf);
	            	if ( A2B_FAILED(status) )
    ...
    )
{
    ...
    A2B_DSCVRY_SEQGROUP1( plugin->ctx,
                          "periphSpiCfgProcessing for nodeAddr %hd",
                          &nodeAddr );

    for (numExec = 0u; numExec < plugin->periph.spiUnitCount[nodeAddr]; numExec++)
    {
        A2B_DSCVRY_SEQGROUP2( plugin->ctx,
                          "nodeAddr %hd, cfgBlock %bd",
                          &nodeAddr,
        ...
        if ( A2B_FAILED(status) )
        {
            A2B_DSCVRY_ERROR1( plugin->ctx, "periphSpiCfgProcessing",
                               "Failed to read SPI EEPROM Cfg Blocks %bd Header",
							   rpBuf );
            A2B_DSCVRY_SEQEND( plugin->ctx );
            A2B_DSCVRY_SEQEND( plugin->ctx );
            A2B_DSCVRY_SETERROR( plugin, A2B_EC_IO );
        ...
        payloadLen = (a2b_UInt16)((a2b_UInt16)(((a2b_UInt16)rpBuf[0]) << (a2b_UInt16)8u) |
                       ((a2b_UInt16)rpBuf[1])) & (a2b_UInt16)0xFFFu;
        payloadDataLen=payloadLen;

        /* Read the payload if needed */
        if ( (a2b_UInt8)CFGBLK_TYPE_C == cfgType )
        {
			crc8 = a2b_crc8(rpBuf, 0u, 2u);
			...
			/* Active delay for SPI peripherals, on timeout we would not know if this delay was initiated by I2C or SPI */
        ...
        else
        {
            if ( payloadLen > A2B_MAX_PERIPHERAL_BUFFER_SIZE )
            {
                A2B_DSCVRY_ERROR2( plugin->ctx, "periphSpiCfgProcessing",
                       "EEPROM Cfg Block %bd Payload (%hd) Exceeds the Buffer",
                       &numExec,
                       &payloadLen );
			...
			if ( (a2b_UInt8)CFGBLK_TYPE_B == cfgType )
			{
				 ...
				 * calculation after the payload has been read.
				 */
				crc8 = a2b_crc8(rpBuf, 0u, 3u);
			}

			periphNode->addr += 3u;
			rpBuf += 3u; /* first three bytes are header */
			...
			{
				A2B_DSCVRY_ERROR1( plugin->ctx, "periphSpiCfgProcessing",
								   "Failed to read SPI EEPROM Cfg Block %bd Header",
								   &numExec );
				A2B_DSCVRY_SEQEND( plugin->ctx );
				A2B_DSCVRY_SEQEND( plugin->ctx );
				A2B_DSCVRY_SETERROR( plugin, A2B_EC_IO );
			...
			{
				/* Complete the CRC calculation */
				crc8 = a2b_crc8Cont(rpBuf, crc8, 0u,
									((a2b_UInt32)payloadLen-1u));

				if ( crc8 != rpBuf[payloadLen-1u] )
				{
					A2B_DSCVRY_ERROR1( plugin->ctx, "periphSpiCfgProcessing",
									   "Bad Type B Cfg Block %bd CRC",
									   &numExec );
			...
			if ( (a2b_UInt8)CFGBLK_TYPE_G == cfgType )
			{
				...
				{
					/* Check if this is a SPI to SPI Read */
					if((a2b_UInt16)regAddr & 0x80u)
					{
						eSpiMode = (A2B_SPI_MODE)((a2b_UInt16)regAddr & 0x0Fu);
						IfAddr = ((a2b_UInt16)regAddr >> 4u) & 0x7u;
						spiCmd = (a2b_UInt16)a2b_getSpiCmd(mCtx, eSpiMode, A2B_TRUE);
						status = a2b_SpiCmdsAndPeriWriteRead(mCtx, nodeAddr, (a2b_SpiCmd)spiCmd, IfAddr, payloadDataLen, rpBuf, 0u, A2B_NULL , A2B_MAX_LENINBYTES_REMOTESPITOSPI);
					}
				}
			}
			else
			...
			if ( A2B_FAILED(status) )
			{
				if ( plugin->overrides[0] & A2B_MPLUGIN_IGN_PERIPH_ERR )
				{
					/* We are going to ignore this error and
					   continue processing */
					A2B_DSCVRY_ERROR1( plugin->ctx, "periphSpiCfgProcessing",
						   "Override Set, Ignoring SPI EEPROM Cfg Block %bd Error",
						   &numExec );
				}
				else
				{
					A2B_DSCVRY_ERROR1( plugin->ctx, "periphSpiCfgProcessing",
						   "SPI EEPROM Cfg Block %bd Peripheral Write Error",
						   &periphNode->cfgIdx );
