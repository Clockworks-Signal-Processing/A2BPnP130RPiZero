...
static a2b_HResult
a2b_mailBoxWrite(a2b_Plugin*  plugin,a2b_UInt8* pWriteBuf,
...
static void a2b_SendCommChEventNotification(a2b_Plugin* plugin, A2B_COMMCH_EVENT eCommChEvent, a2b_Int16 NodeAddr,  a2b_CommChMsg * pMsg, A2B_COMMCH_FRAMING eFraming);
...
static void a2b_pluginInterrupt(struct a2b_StackContext* ctx,
    ...
    a2b_UInt8                   intrSrc,
    a2b_UInt8                   intrType);
static void a2b_pluginCustAuthInit(struct a2b_StackContext* ctx, a2b_Plugin* plugin);
    ...
    )
{
	...
	plugin->p244xCPNetConfigStruct = (ADI_A2B_244x_CP_NETCONFIG *)discPayload->req.pCPRegConfig;
#endif
		...
		a2b_UInt8 nDataSize, a2b_Int16 nSlvNodeAddr,  a2b_UInt8 nMBoxNo)
{
	...
	status = a2b_regWriteRead(plugin->ctx, nSlvNodeAddr, 1u, &wBuf, (a2b_UInt16)nDataSize, pReadBuf);
	if(A2B_FAILED(status))
	{
		A2B_TRACE1((plugin->ctx, (A2B_TRC_DOM_PLUGIN | A2B_TRC_LVL_ERROR),
					"Exit: %s execute(): Mailbox Write Failed",
...
static void a2b_CommChCallBk(void* pCbParam, a2b_CommChMsg *pMsg, A2B_COMMCH_EVENT eEventType, a2b_Int8 nNodeAddr)
{
	...
	if(pMsg->nMsgId >= A2B_COMMCH_MSG_CUSTOM_START)
	{
    	...
    	if(bRet == A2B_TRUE)
    	{
    		(void)adi_a2b_CommChMstrGetFraming(plugin->commCh.commChHnd[nCommChInstNo], &eFraming);
    	}
    	else
    	{
    		eFraming = A2B_COMMCH_FRAMIMG_UNKNOWN;
    	}

		/* New application event .Notify event to higher layer */
		a2b_SendCommChEventNotification(plugin, eEventType, nNodeAddr, pMsg, eFraming);
	}
	else
	{
...
static void a2b_SendCommChEventNotification(a2b_Plugin* plugin, A2B_COMMCH_EVENT eCommChEvent, a2b_Int16 NodeAddr, a2b_CommChMsg *pMsg, A2B_COMMCH_FRAMING eFraming)
{
	...
	pCommChEventInfo->eFraming  = eFraming;

	/* Make best effort delivery of notification */
	status = a2b_msgRtrNotify(pCommChnotifyMsg);
	if ( A2B_FAILED(status) )
	{
		 A2B_TRACE1((plugin->ctx, (A2B_TRC_DOM_PLUGIN | A2B_TRC_LVL_ERROR), "Exit: %s: Communication channel Event Notify failed", A2B_MPLUGIN_PLUGIN_NAME));
	}
...
static void a2b_pluginCommChInit(struct a2b_StackContext* ctx, a2b_Plugin* plugin)
{
     ...
     */
    plugin->commCh.commChMboxHnd = a2b_stackCtxMailboxAlloc(plugin->ctx, A2B_JOB_PRIO0 );

    if ( A2B_NULL == plugin->commCh.commChMboxHnd )
    {
        A2B_TRACE1( (plugin->ctx, (A2B_TRC_DOM_PLUGIN | A2B_TRC_LVL_ERROR), "Exit: %s: a2b_pluginCommChInit: failed to create communication channel mailbox", A2B_MPLUGIN_PLUGIN_NAME));
    }
	...
	oCommChConfig.eFraming  = plugin->commCh.eFraming;
    ...
    )
{
    ...
    a2b_NetPostDiscovery*       netPostDisc;
    ...
    a2b_CommChFramingInfo		*pCommChFramingInfo;
    A2B_COMMCH_RET				nCommChRet;
    a2b_Bool 					bRet;
 #endif
    ...
    {
        A2B_TRACE1((ctx, (A2B_TRC_DOM_PLUGIN | A2B_TRC_LVL_ERROR), "Exit: %s execute(): Internal error", A2B_MPLUGIN_PLUGIN_NAME));
        return A2B_EXEC_COMPLETE;
    }

    A2B_TRACE2((ctx, (A2B_TRC_DOM_PLUGIN | A2B_TRC_LVL_TRACE1), "Enter: %s execute(%ld):", A2B_MPLUGIN_PLUGIN_NAME, &cmd));
    ...
    switch ( cmd )
    {
        ...
        case A2B_MSGREQ_NET_RESET:
            ...
            (void)a2b_regWrite( ctx,A2B_NODEADDR_MASTER, 2u, &wBuf);
        ...
        case A2B_MSGREQ_NET_DISCOVERY:
            ...
            /* Initialize the custom node authentication related parameters from bdd */
            a2b_pluginCustAuthInit(ctx, plugin);

            netDisc = (a2b_NetDiscovery*)a2b_msgGetPayload(msg);

            if ( A2B_SUCCEEDED(status) )
            {
				/* Initializing the first time discovery flag */
				plugin->discovery.bFrstTimeDisc = netDisc->req.bFrstTimeDisc;
				//plugin->discovery.nRediscTimeOutForGndFaultDetection = netDisc->req.nRediscTimeOutForGndFaultDetection;
...
#ifdef A2B_FEATURE_COMM_CH
        case A2B_MSGREQ_COMMCH_SET_FRAMING:
        	/* Get Message Payload information*/
        	pCommChFramingInfo = (a2b_CommChFramingInfo*)a2b_msgGetPayload(msg);

        	bRet = a2b_GetCommChInstIdForSlvNode(plugin, pCommChFramingInfo->req.nSlvNodeAddr, &nCommChInstNo);
        	if(bRet == A2B_TRUE)
        	{
				nCommChRet = adi_a2b_CommChMstrSetFraming(plugin->commCh.commChHnd[nCommChInstNo], pCommChFramingInfo->req.eFraming);
				if(nCommChRet == A2B_COMMCH_FAILED)
				{
					/* Framing request over Communication channel failed, Notify error event to higher layer
					 * Since there is no msg involved at this stage the pMsg = A2B_NULL
					 * */
					a2b_SendCommChEventNotification(plugin, A2B_COMMCH_EVENT_SET_FRAMING_FAILURE, pCommChFramingInfo->req.nSlvNodeAddr, A2B_NULL, A2B_COMMCH_FRAMIMG_UNKNOWN);
					A2B_TRACE1((ctx, (A2B_TRC_DOM_PLUGIN | A2B_TRC_LVL_ERROR), "Exit: %s execute(): Communication Channel Set framing failed", A2B_MPLUGIN_PLUGIN_NAME));
				}
				else
				{
					/* Record the response if framing API is success */
					(void)adi_a2b_CommChMstrGetFraming(plugin->commCh.commChHnd[nCommChInstNo], &pCommChFramingInfo->resp.eFraming);
					pCommChFramingInfo->resp.nSlvNodeAddr 	= pCommChFramingInfo->req.nSlvNodeAddr;

					a2b_SendCommChEventNotification(plugin, A2B_COMMCH_EVENT_SET_FRAMING_SUCCESS, pCommChFramingInfo->req.nSlvNodeAddr, A2B_NULL, pCommChFramingInfo->resp.eFraming);
					A2B_TRACE1((ctx, (A2B_TRC_DOM_PLUGIN | A2B_TRC_LVL_ERROR), "Exit: %s execute(): Communication Channel Set framing success", A2B_MPLUGIN_PLUGIN_NAME));
				}
        	}
        	else
        	{
        		/* No communication channel instance mapped for the slave node, Notify error event to higher layer
        		 * Since there is no msg involved at this stage the pMsg = A2B_NULL
        		 * */
        		a2b_SendCommChEventNotification(plugin, A2B_COMMCH_EVENT_FAILURE, pCommChFramingInfo->req.nSlvNodeAddr, A2B_NULL, A2B_COMMCH_FRAMIMG_UNKNOWN);
				A2B_TRACE1((ctx, (A2B_TRC_DOM_PLUGIN | A2B_TRC_LVL_ERROR), "Exit: %s execute(): Communication Channel : Instance for slave node address not available", A2B_MPLUGIN_PLUGIN_NAME));
        	}
        ...
        case A2B_MSGREQ_COMMCH_GET_FRAMING:
        	/* Get Message Payload information*/
        	pCommChFramingInfo = (a2b_CommChFramingInfo*)a2b_msgGetPayload(msg);
        	...
        	if(bRet == A2B_TRUE)
        	{
        		nCommChRet = adi_a2b_CommChMstrGetFraming(plugin->commCh.commChHnd[nCommChInstNo], &pCommChFramingInfo->resp.eFraming);
				if(nCommChRet == A2B_COMMCH_FAILED)
				{
					/* Framing request over Communication channel failed, Notify error event to higher layer
					 * Since there is no msg involved at this stage the pMsg = A2B_NULL
					 * */
					a2b_SendCommChEventNotification(plugin, A2B_COMMCH_EVENT_GET_FRAMING_FAILURE, pCommChFramingInfo->req.nSlvNodeAddr, A2B_NULL, A2B_COMMCH_FRAMIMG_UNKNOWN);
					A2B_TRACE1((ctx, (A2B_TRC_DOM_PLUGIN | A2B_TRC_LVL_ERROR), "Exit: %s execute(): Communication Channel Get framing failed", A2B_MPLUGIN_PLUGIN_NAME));
				}
				else
				{
					pCommChFramingInfo->resp.nSlvNodeAddr = pCommChFramingInfo->req.nSlvNodeAddr;
					 ...
					 * Since there is no msg involved at this stage the pMsg = A2B_NULL
					 * */
					a2b_SendCommChEventNotification(plugin, A2B_COMMCH_EVENT_GET_FRAMING_SUCCESS, pCommChFramingInfo->req.nSlvNodeAddr, A2B_NULL, pCommChFramingInfo->resp.eFraming);
					A2B_TRACE1((ctx, (A2B_TRC_DOM_PLUGIN | A2B_TRC_LVL_ERROR), "Exit: %s execute(): Communication Channel Get framing success", A2B_MPLUGIN_PLUGIN_NAME));
				}
        	}
        	else
        	{
        ...
        case A2B_MSGREQ_COMMCH_SEND_MSG:
        	...
        	if(bRet == A2B_TRUE)
        	{
				...
				if(nCommChRet == A2B_COMMCH_FAILED)
				{
					/* Transmission request over Communication channel failed, Notify error event to higher layer */
					a2b_SendCommChEventNotification(plugin, A2B_COMMCH_EVENT_FAILURE, pCommChTxInfo->req.nSlvNodeAddr, pCommChTxInfo->req.pTxMsg, A2B_COMMCH_FRAMIMG_UNKNOWN);
					A2B_TRACE1((ctx, (A2B_TRC_DOM_PLUGIN | A2B_TRC_LVL_ERROR), "Exit: %s execute(): Communication Channel Send Message failed", A2B_MPLUGIN_PLUGIN_NAME));
				}
        	}
        	else
        	{
        ...
        case A2B_MSGREQ_SEND_MBOX_DATA:
        	...
        	status = a2b_mailBoxWrite(plugin, pMboxTxInfo->req.pwBuf, pMboxTxInfo->req.nDataSz,pMboxTxInfo->req.nSlvNodeAddr, pMboxTxInfo->req.nMbox);
...
#endif
#ifdef A2B_FEATURE_PARTIAL_DISC
        case A2B_MSGREQ_NET_POST_DISCOVERY:
            if (plugin->bdd->policy.bEnablePartialDisc == A2B_TRUE)
            {
                netPostDisc = (a2b_NetPostDiscovery*)a2b_msgGetPayload(msg);

                if (netPostDisc->req.bBusDropDetected == A2B_TRUE)
                {
                    /* Update the no. of nodes to dropped node */
                    plugin->discovery.dscNumNodes = netPostDisc->req.nBusDropNodeId;
                    plugin->discovery.simpleNodeCount = netPostDisc->req.nBusDropNodeId;
                    plugin->discovery.dscNumPartialNodes = netPostDisc->req.nBusDropNodeId;
                }

                ret = a2b_dscvryPartial(plugin);
            }
            else
            {
                A2B_TRACE1((ctx, (A2B_TRC_DOM_PLUGIN | A2B_TRC_LVL_ERROR),
                    "Exit: %s execute(): Partial Discovery is not enabled in bus configuration",
                    A2B_MPLUGIN_PLUGIN_NAME));
            }
...
#endif
        default:
            A2B_TRACE2((ctx, (A2B_TRC_DOM_PLUGIN | A2B_TRC_LVL_WARN), "%s execute(%ld): Unhandled command", A2B_MPLUGIN_PLUGIN_NAME, &cmd));
            break;
    }

    A2B_TRACE3((ctx, (A2B_TRC_DOM_PLUGIN | A2B_TRC_LVL_TRACE1), "Exit: %s execute(%ld): 0x%lX", A2B_MPLUGIN_PLUGIN_NAME, &cmd, &ret));
    ...
    if ( A2B_NULL != plugin )
    {
        switch ( intrType )
        {
            case A2B_ENUM_INTTYPE_DSCDONE:
                ...
                if ( (a2b_pwrDiagIsActive(plugin)) || (a2b_pwrDiagIsActiveAD243x(plugin)) )
                {
                    a2b_pwrDiagDiagnose(plugin, intrSrc, intrType);
                }
                else if ( plugin->discovery.inDiscovery )
                {
                    bRet = a2b_dscvryNodeDiscovered(plugin);
                    a2b_CheckIfAD2430_8NodeMasterPrsnt(plugin, &bIsAD2430_8MasterPresent);
                    if ((bRet == A2B_FALSE) && (bIsAD2430_8MasterPresent == A2B_TRUE) && (plugin->discovery.bAd2430VmtrFault == A2B_TRUE))
                    {
                        plugin->discovery.bAd2430VmtrFault = false;
                        /* Condition called to explicitly handle power fault - BP to VBat in low voltage conditions*/
                        a2b_pwrDiagDiagnose(plugin, A2B_BITM_INTSRC_SLVINT, A2B_ENUM_INTTYPE_PWRERR_NLS_VBAT);
                    }
			...
			case A2B_ENUM_INTTYPE_PWRERR_OTH:
			...
			case A2B_ENUM_INTTYPE_MSTR_RUNNING:
				/* update the flag indicating that master running interrupt has happened */
				plugin->discovery.bIsMstrRunning = true;
#ifdef DISABLE_PWRDIAG
            	if(!plugin->bDisablePwrDiag)
            	{
#endif
...
/*!****************************************************************************
*
*  \b              a2b_pluginCustAuthInit
*
*  This function initializes the custom node authentication related parameters from bdd
*
*  \param          [in]    ctx	    stack context
*
*  \param          [in]    plugin	plugin
...
static void a2b_pluginCustAuthInit(struct a2b_StackContext* ctx, a2b_Plugin* plugin)
{
	...
	/* Clear the custom auth structure */
	(void)a2b_memset( &plugin->customAuth[0U], 0U, (sizeof(a2b_CustomAuth)*A2B_CONF_MAX_NUM_SLAVE_NODES) );

	/* Initialize the retry count from bdd
	 * Index starts from 1 to exclude the master node
	 * */
	for(nIdx=1U ; nIdx<plugin->bdd->nodes_count ; nIdx++)
	{
